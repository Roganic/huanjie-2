"""One persistence API over the complete session snapshot service."""
...
async def load_by_id(save_id: str):
    try:
        result = gs.load_saved_game(save_id=save_id)
    except Exception as exc:
        raise HTTPException(400, "存档损坏或格式不兼容。") from exc
    if result is None:
        raise HTTPException(404, "找不到存档。")
    return result
