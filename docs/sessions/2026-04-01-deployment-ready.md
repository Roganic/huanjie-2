...
- 本次重试已撤销 `docs/tech-choices.md` 的会话差异，避免继续触碰 integrator-only 文件
...
- 重试会话再次执行 `cd app/backend && python3 -m pytest tests/test_deployment_flow.py -q`，结果 `1 passed`
- 重试会话再次执行 `cd app/frontend && npm run build:pages`，构建通过
